...
module.exports = (app) => {
  ...
  app.get("/atualiza", async (requisicao, resposta) => {
    ...
    do {
      ...
      await vendas.tratavendas(registro, idNotaFiscal);
      controla({
        "================================================ ATUALIZA MONGODB ======================": new Date(),
      });
      // Atualiza o registro no MongoDB
      await updateMongoDb(registro._id);
      //
      i++;
    ...
    globalFINAL = now();
    globalRESULTADOATUALIZA.unshift({
      "Tempo total de Processamento ": (globalFINAL - globalINICIO) / 1000,
    });
    globalRESULTADOATUALIZA.unshift({
      "Tempo total de Processamento por registro  ":
        (globalFINAL - globalINICIO) / 1000 / limit,
    });
  ...
  app.get("/apaga", async (req, res) => {
    ...
    var mensagemfinal = {
      "Total de registros": resultado.n,
