...
async function registraVendaBubble(jsonVenda) {
  return new Promise((resolve, reject) => {
    ...
    axios
      .post(
        "https://copiagexsyt.bubbleapps.io/version-test/api/1.1/wf/postvenda/",
        jsonVenda
      )
      .then((respostaBubble) => {
...
async function extraiVendas(registropassado, idNotaFiscal) {
  ...
  for (let index = 0; index < quantidade; index++) {
    registroTemporario = {};
    let idVendavel = await vendavel.idVendavel(registropassado.produto[index]);
    registroTemporario = {
...
async function tratavendas(registro, idnotafiscal) {
  ...
  const vendasExtraida = await extraiVendas(registro, idnotafiscal);
